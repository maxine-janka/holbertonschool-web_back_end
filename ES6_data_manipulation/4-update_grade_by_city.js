export default function updateStudentGradeByCity(studentList, city, newGrades) {
  ...
  // if there is a match with student id's, store the grade object in studentGradeObject
  const updatedStudents = studentsInCity.map((student) => {
    const studentGradeObject = newGrades.find((grade) => grade.studentId === student.id);

    // If grade object exists from prev match, take the grade property and store in grade,
    // Otherwise set as NA
    let grade;
